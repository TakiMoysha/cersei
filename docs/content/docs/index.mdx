...
<Callout type="info">
Cersei is part of the **[Atlas](https://tryatlas.cc/)** project — the open agent stack. Join the conversation on [Discord](https://discord.gg/GmnFggaPfP).
</Callout>

<Callout type="warn">
**New — [AgentRL](/docs/agentrl-overview): self-evolving agents.** Agents that trace their own failures, propose fixes in isolated sandboxes, promote the one that passes verification, and **register it as a reusable, programmable tool**. They get cheaper and more capable on recurring problems over time. Drive it with the `AgentRlRunner` trait and author sub-agents in the **[AgentTemplate](/docs/agentrl-template)** language. [Read the overview →](/docs/agentrl-overview)
</Callout>

### Featured on Dreams of Code

<div className="relative w-full overflow-hidden rounded-xl border border-neutral-800 my-6" style={{ aspectRatio: '16 / 9' }}>
  <iframe
    src="https://www.youtube.com/embed/WkpEuogh3OY"
    title="Cersei on Dreams of Code"
...
<Cards>
  <Card title="AgentRL — Self-Evolving Agents" href="/docs/agentrl-overview">
    The flagship orchestration layer. Trace failures, propose fixes in sandboxes, promote the verified winner, and register it as a reusable tool. Programmable via the `AgentRlRunner` trait and the AgentTemplate DSL.
  </Card>
...
Cersei is on [crates.io](https://crates.io/crates/cersei):
...
For the Abstract CLI (complete coding agent) — install directly from crates.io:

```bash
cargo install abstract-cli
```

Or one-line install on macOS or Linux without a Rust toolchain (detects OS, installs Rust if needed, builds from source, drops the binary on your PATH):
...
<Cards>
  <Card title="AgentRL" href="/docs/agentrl-overview">
    Self-evolving agents: failure tracing, sandboxed proposals, a tool registry, and the AgentTemplate language.
  </Card>
