...
<Callout type="info">
Beyond text, messages can carry **images, video, audio, and PDFs**. See [Multimodal Input](/docs/multimodal) for the file-passing API and which media each provider accepts.
</Callout>
...
Selecting `--provider deepseek` (or any `deepseek-*` model name) always resolves
...
`DEEPSEEK_BASE_URL` (see [Custom Endpoints](#adding-custom-providers)).
...
For the CLI, any OpenAI-compatible provider's base URL can be overridden with a
...
When an override is in effect, Abstract prints a redacted target line before the
first request — e.g. `provider=openai host=my-custom-api.com model=my-model
key_present=true` — so a misrouted endpoint is obvious instead of surfacing as an
